...
int F(char symbol)
{
    switch (symbol)
    {
        // Prefix ma equal operators mate reversal logic alag hoy chhe
        case '+':
        case '-':
            return 2;
...
int main()
{
    ...
    // Step 1: Input string ne reversed order ma gochvo (Prefix mate aakha logic ulta thashe)
    for (i = 0; i < len; i++)
    {
    ...
    // Stack ni andar pehla ')' push karo (Karan ke string reverse thai gai chhe)
    PUSH(')');

    // Reversed expression ni chhelle '(' jodi do jethi process end thai sake
    strcat(REVERSED_INFIX, "(");

    // Intermediate Prefix string ne khali (empty) karo
    PREFIX_TEMP[0] = '\0';

    // Loop chalu karva mate index fresh set karo ane pehlo character lo
    i = 0;
    NEXT = REVERSED_INFIX[i++];

    // Jya sudhi string puri na thay (null na aave) tya sudhi loop fershe
    while (NEXT != '\0')
    {
        ...
        // Jo stack ni priority input karta vadhare hoy toh pop kari ne result ma nakho
        ...
        // Jo banne ni precedence sarakhi na hoy toh navo character stack ma push karo
        if (G(PEEP()) != F(NEXT))
        {
            PUSH(NEXT);
        }
        ...
        {
            // Jo precedence sarakhi hoy (jem ke banne matching brackets hoy) toh khali pop kari do
            POP();
        }

        // Pachi no character read karo
        NEXT = REVERSED_INFIX[i++];
    }
    ...
    // Chhelle check karo ke rank exactly 1 chhe ke nahi ane stack khali chhe ke nahi
    if (TOP != -1 || RANK != 1)
    {
        printf("\nINVALID EXPRESSION");
    }
    ...
    {
        // Final Output Step: Intermediate string ne pachi thi reverse karo true Prefix melavva mate
        printf("\nPrefix Expression : ");
